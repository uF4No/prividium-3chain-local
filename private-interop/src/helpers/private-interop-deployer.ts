...
export interface DeployOptions {
  ...
  skipFunding?: boolean;
  /** Gas overrides for the initial funding transaction. */
  fundingGasOverrides?: { gasPrice?: ethers.BigNumber; gasLimit?: number; type?: number };
...
export async function deployPrivateInteropStack(
  ...
  if (!opts.skipFunding) {
    ...
    if (deployerBalance.lt(ethers.utils.parseEther("1"))) {
      const baseGasPrice = (opts.deployGasOverrides?.gasPrice || (await provider.getGasPrice())).mul(2);
      const fundingOverrides = {
        gasPrice: opts.fundingGasOverrides?.gasPrice || baseGasPrice,
        gasLimit: opts.fundingGasOverrides?.gasLimit || opts.deployGasOverrides?.gasLimit || 30_000_000,
        type: opts.fundingGasOverrides?.type ?? opts.deployGasOverrides?.type ?? 0,
      };
...
export async function registerRemoteRouters(
  ...
  for (const chain of chains) {
    ...
    const baseGasPrice = gasOverrides?.gasPrice || (await provider.getGasPrice()).mul(2);
    const overrides = {
      gasPrice: baseGasPrice,
      gasLimit: gasOverrides?.gasLimit || 10_000_000,
      type: gasOverrides?.type ?? 0,
    };
